...
$client = new \GuzzleHttp\Client(['verify' => false, 'base_uri' => 'https://h5.qzone.qq.com', 'headers' => ['Cookie' => $cookies]]);
...
foreach ($all_user->data->rank as $user) {
    ...
    if ($user->resource_state == 'normal') {
        ...
        if (json_decode($r->getBody())->data->code == 0) {
            echo $echohead . '成功偷取' . $user->nick . '的糖果' . PHP_EOL;
        }
    }
}
...
foreach ($pets as $pet) {
    ...
    if ($hasnewbaba >= 1) {
        ...
        if (json_decode($r->getBody())->data->code == 0) {
            echo $echohead . '成功生成宠物' . $petname . '的粑粑' . PHP_EOL;
        } else {
            echo $echohead . '给宠物' . $petname . '生成粑粑失败' . PHP_EOL;
        }
